...
import { ArrowLeft, ChevronRight, Landmark, Loader2, Search } from 'lucide-react';
...
const InvestAndEarnPage: React.FC<InvestAndEarnPageProps> = ({ onAssetClick }) => {
  ...
  const basePath = '/dashboard/copy';

  const normalizedType = (marketType ?? '').toLowerCase();
  const activeType: 'stocks' | null = normalizedType === 'stocks' ? 'stocks' : null;
  ...
  const filteredAssets = useMemo(() => {
    ...
    return [...marketAssets]
      .filter((asset) => {
        ...
        const isTypeMatch = STOCK_TYPES.has(type);
  ...
  return (
    <div className="px-4 py-6 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <header>
        <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">Invest & Earn</p>
        <h2 className="text-2xl font-black text-white tracking-tight">Buy Stocks</h2>
