...
const LandingAuthModal: React.FC<LandingAuthModalProps> = ({
  ...
  const inputClass = 'w-full bg-[#0a0a0a] border border-white/5 rounded-2xl py-4 px-4 text-sm font-bold text-white focus:outline-none transition-all placeholder:text-zinc-700 focus:border-emerald-500/40';
  const inputWithIconClass = 'w-full bg-[#0a0a0a] border border-white/5 rounded-2xl py-4 pl-12 pr-4 text-sm font-bold text-white focus:outline-none transition-all placeholder:text-zinc-700 focus:border-emerald-500/40';
  const linkButtonClass = 'text-[10px] font-black uppercase tracking-widest text-emerald-500 hover:text-emerald-400';
  const primaryActionClass = 'w-full py-4 text-black font-black rounded-2xl uppercase tracking-[0.2em] text-sm shadow-xl active:scale-[0.98] transition-all bg-emerald-500 hover:bg-emerald-400 shadow-emerald-500/10';
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="w-full max-w-md bg-[#121212] border border-white/10 rounded-[40px] p-8 md:p-12 shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="absolute top-0 right-0 w-32 h-32 blur-[60px] rounded-full -translate-y-1/2 translate-x-1/2 bg-emerald-500/10" />
        <div className="absolute bottom-0 left-0 w-32 h-32 blur-[60px] rounded-full translate-y-1/2 -translate-x-1/2 bg-emerald-500/10" />
        ...
        <div className="flex flex-col items-center mb-10">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center shadow-lg mb-6 shadow-emerald-500/20">
        ...
        {isSubmitting ? (
          <div className="py-12 flex flex-col items-center justify-center animate-in fade-in zoom-in-95">
            <div className="relative mb-8">
              <Loader2 size={48} className="animate-spin text-emerald-500" strokeWidth={3} />
              <div className="absolute inset-0 blur-2xl animate-pulse bg-emerald-500/20" />
            </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                <div className="space-y-4">
                  <div className="relative group">
                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-600 transition-colors group-focus-within:text-emerald-500" size={20} />
                    <input
                      type="email"
                      required
                  ...
                  <div className="relative group">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-600 transition-colors group-focus-within:text-emerald-500" size={20} />
                    <input
                      type="password"
                      required
            ...
            {authView === 'signup' && (
              <form onSubmit={handleSignup} className="space-y-4">
                ...
                <p className="text-center text-[10px] font-bold text-zinc-600">
                  ALREADY REGISTERED?{' '}
                  <button type="button" onClick={() => switchAuthView('login')} className="font-black uppercase tracking-widest text-emerald-500">
                    LOGIN
                  </button>
                </p>
              </form>
            ...
            {authView === 'verify' && (
              <form onSubmit={handleVerifyOtp} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full py-3 border border-white/10 rounded-2xl text-[10px] font-black text-zinc-300 uppercase tracking-widest transition-colors hover:border-emerald-500/40"
                >
                  Resend OTP
                </button>
                <p className="text-center text-[10px] font-bold text-zinc-600">
                  WRONG ACCOUNT?{' '}
                  <button type="button" onClick={() => switchAuthView('signup')} className="font-black uppercase tracking-widest text-emerald-500">
                    SIGN UP AGAIN
                  </button>
                </p>
              </form>
            ...
            {authNotice && (
              <p className="text-xs font-bold text-center text-emerald-400">{authNotice}</p>
            )}
            {currentError && (
